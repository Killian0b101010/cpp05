...
/*   Updated: 2025/12/12 18:01:11 by kiteixei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executForm(AForm const &form) {
  try {
    std::cout << Name << " try execut " << form.getName() << std::endl;
    form.execute(*this);
    
  } catch (std::exception &e) {
    std::cout << Name << " couldn’t sign " << form.getName()
              << " because bureaucrat is lvl " << getGrade() << " contract to execute lvl "
              << form.gradeToExecute() << " " << e.what() << std::endl;
  }
}
